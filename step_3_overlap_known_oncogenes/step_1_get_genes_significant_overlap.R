...
# output: CSV with which genes are shared by which cancer types, filled with NA at the bottom
#          of columns that aren't full
library(tidyverse)
library(reshape2)
...
max.len = max(length(uall), length(ucr_lung), length(uen_lung), length(ucr_en), length(uen_genes), length(ucr_genes), length(ulung_genes))
Uall = c(uall, rep(NA, max.len - length(uall)))
Ucr_lung = c(ucr_lung, rep(NA, max.len - length(ucr_lung)))
Uen_lung = c(uen_lung, rep(NA, max.len - length(uen_lung)))
Ucr_en = c(ucr_en, rep(NA, max.len - length(ucr_en)))
Uen_genes = c(uen_genes, rep(NA, max.len - length(uen_genes)))
Ulung_genes = c(ulung_genes, rep(NA, max.len - length(ulung_genes)))
Ucr_genes = c(ucr_genes, rep(NA, max.len - length(ucr_genes)))

max.len = max(length(dall), length(dcr_lung), length(den_lung), length(dcr_en), length(den_genes), length(dcr_genes), length(dlung_genes))
Dall = c(dall, rep(NA, max.len - length(dall)))
Dcr_lung = c(dcr_lung, rep(NA, max.len - length(dcr_lung)))
Den_lung = c(den_lung, rep(NA, max.len - length(den_lung)))
Dcr_en = c(dcr_en, rep(NA, max.len - length(dcr_en)))
Den_genes = c(den_genes, rep(NA, max.len - length(den_genes)))
Dlung_genes = c(dlung_genes, rep(NA, max.len - length(dlung_genes)))
Dcr_genes = c(dcr_genes, rep(NA, max.len - length(dcr_genes)))


upcsv <- data.frame(Uall, Ucr_lung, Uen_lung, Ucr_en, Uen_genes, Ucr_genes, Ulung_genes, stringsAsFactors=FALSE)
write.csv(upcsv, file = 'upregulation.csv', row.names = FALSE)

downcsv <- data.frame(Dall, Dcr_lung, Den_lung, Dcr_en, Den_genes, Dcr_genes, Dlung_genes, stringsAsFactors=FALSE)
write.csv(downcsv, file = 'downregulation.csv', row.names = FALSE)
